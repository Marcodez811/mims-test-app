  ...
  <xsl:template name="listProduct">
    <xsl:param name="Nodes"/>
      <table border="0px" cell-padding="0px">
        ...
        <xsl:for-each select="*">
        <tr>
          <td class="interaction" width="55%"><xsl:value-of select="@name"/></td>
          <td class="interaction" width="30%"><xsl:value-of select="@reference"/></td>
          <td class="interaction" width="15%"><xsl:value-of select="@offMarket"/></td>
        </tr>
        </xsl:for-each>
      </table>
  </xsl:template>
  ...
  <xsl:template name="listItem">
    <xsl:param name="Nodes"/>
      <table border="0px" cell-padding="0px">
        ...
        <xsl:for-each select="*">
        <tr>
          <td class="interaction" width="60%"><xsl:value-of select="@name"/></td>
          <td class="interaction" width="20%"><xsl:value-of select="@reference"/></td>
        </tr>
        </xsl:for-each>
      </table>
  </xsl:template>
  ...
  <xsl:template name="listHealthCode">
    <xsl:param name="Nodes"/>
      <table border="0px" cell-padding="0px">
        ...
        <xsl:for-each select="*">
        <tr>
          <td class="interaction" width="20%"><xsl:value-of select="@codeType"/></td>
          <td class="interaction" width="20%"><xsl:value-of select="@code"/></td>
          <td class="interaction" width="60%"><xsl:value-of select="@name"/></td>
        </tr>
        </xsl:for-each>
      </table>
	...
	<xsl:template match="Interaction">
		...
		<!--<xsl:variable name="numLact" select="count(//Lactation/InteractionClass[count(*) &gt; 0])"/>-->
		<xsl:variable name="numLact" select="count(//Lactation/InteractionClass)"/>
...
<div class="container1">
  ...
  <span>
    ...
    <xsl:value-of select="//Result/@expiryDate"/>
				...
				<xsl:if test="$numvn &gt; 0">
					<li>
						<a href="#tab-vn">Tương tác chống chỉ định VN  (<xsl:value-of select="$numvn"/>)</a>
					</li>
				</xsl:if>
	...
	<xsl:template match="ImageData">
		<xsl:variable name="imageSrc" select="/Result/Content/ImageData" />
    	   <img width="250px" height="250px" src="data:image/png;base64,{ $imageSrc }"/>
	</xsl:template>	
			...
			<xsl:otherwise>
				<xsl:for-each select="Route">
					...
					<xsl:if test="$Mode='pregnancy' and Pregnancy|WOCBA">
						<xsl:variable name="DivId"><xsl:value-of select="translate($IntProd1, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ ','abcdefghijklmnopqrstuvwxyz_')"/>
						</xsl:variable>
	...
	<xsl:template match="Lactation">
		...
		<xsl:variable name="DivId">la_<xsl:value-of select="Severity/@ranking"/>_<xsl:value-of select="translate($IntProd, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ .','abcdefghijklmnopqrstuvwxyz__')"/>_la</xsl:variable>
		<xsl:element name="div">
			<xsl:attribute name="id">
				<xsl:value-of select="$DivId"/>
			</xsl:attribute>
			...
			<div>
				...
				<p>
				<label style="font-size:12px;font-weight:bold;margin-up:-410px">Description<br/></label>
			...
			<xsl:element name="div">
				...
				<h3>
					<a href="#">
						...
						<xsl:value-of select="$IntHealthCode"/> (<xsl:value-of select="$IntHealthCodeDesc"/>)</a>
				</h3>
				<div>
...
<xsl:template match="//Interaction/DANH_SACH_TUONG_TAC">
		...
		<xsl:variable name="Int3" select="MucDoNghiemTrong"/>
			<xsl:element name="div">
				...
				<h4 class="subheading">Mức độ tương tác</h4>
					<xsl:value-of select="MucDoNghiemTrong"/><br/>
				<h4 class="subheading">Cơ chế tương tác</h4>
					<xsl:value-of select="CoCheTuongTac"/><br/>
				
				<h4 class="subheading">Hậu quả tương tác</h4>
					<xsl:value-of select="HauQuaCuaTuongTac"/><br/>
				...
				<xsl:value-of select="TaiLieuThamKhao"/><br/><br/><br/>
				...
				<p style="font-size: 9px;font-style: italic;"><xsl:value-of select="TuyenBoMienTruTrachNhiem"/>
				</p>
				...
				<xsl:for-each select="*">
					<xsl:choose>
						<xsl:when test="name()='CrossSensitive'">
							<xsl:variable name="DivId">c_<xsl:value-of select="translate($IntProd1, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ ','abcdefghijklmnopqrstuvwxyz_')"/>
							</xsl:variable>
							<li>The patient has a known history of allergic reaction to SubstanceClass, <em>
						...
						<xsl:when test="CrossSensitive">
							...
							<li>The patient has a known history of allergic reaction to <xsl:value-of select="name()"/>, <em>
									<xsl:value-of select="@name"/>
								</em>.<ul class="no-bullet-no-indent">
									<li>
										<em>
	...
	<xsl:template match="DoseCheck|CombinedDoseCheck">
		<xsl:for-each select="*[count(Warnings) &gt; 0]">
			...
			<xsl:variable name="DivId"><xsl:value-of select="concat ('dc_', translate($IntProd, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ ','abcdefghijklmnopqrstuvwxyz_'), '_dc')"/>
			</xsl:variable>
				...
				<xsl:element name="div">
					...
					<h3>
						<a>
							...
							<xsl:value-of select="$DupPair"/> [ATC Code: <xsl:value-of select="$ATCCode"/>]</a>
					</h3>
				</xsl:element>
			</xsl:for-each>
		</xsl:for-each>
...
<xsl:choose>
    ...
    <xsl:when test="count(//Interaction/DuplicateIngredient/Warning/Molecule/Molecule/*) &gt;= 1">
        <!-- Add XSLT instructions for the second case -->
			...
			<xsl:for-each select="./Molecule/Molecule">
				...
				<xsl:element name="div">
					<xsl:attribute name="id">d_<xsl:value-of select="$DuplicateLevel"/>_<xsl:value-of select="translate($DupPair, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ ','abcdefghijklmnopqrstuvwxyz_')"/>_<xsl:value-of select="translate($DupPair1, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ ','abcdefghijklmnopqrstuvwxyz_')"/>
					</xsl:attribute>
					...
					<h3>
						...
						<xsl:value-of select="$DupPair"/> [<xsl:value-of select="@name"/>] </a>
					</h3>
				</xsl:element>
				</xsl:for-each>
	...
	<!-- Lookup tables -->
	<mims:d2d_sev_levels>
		<mims:d2d_sev_level name="2">The interaction may occur based on the mechanism of action of the co-administered medicines. Be alert for increased or decreased effect, depending on the combination of medicines.</mims:d2d_sev_level>
		<mims:d2d_sev_level name="3">Clinical effects of the interaction are limited and may be bothersome but would not usually require a major change to therapy. The patient should be monitored for the possible manifestations of the interaction.</mims:d2d_sev_level>
		<mims:d2d_sev_level name="4">These medications may interact resulting in the potential deterioration of the patient's condition. The patient should be monitored for the possible manifestations of the interaction. Medical intervention or a change in therapy may be required.</mims:d2d_sev_level>
		<mims:d2d_sev_level name="5">The interaction between these medications may be life-threatening or may cause permanent damage. These medications are not usually used concurrently; medical intervention may be required.</mims:d2d_sev_level>
	</mims:d2d_sev_levels>
	<mims:d2h_sev_levels>
		<mims:d2h_sev_level name="3">Absolute contraindication; the use of the drug is completely inadvisable and should be avoided.</mims:d2h_sev_level>
		<mims:d2h_sev_level name="2">Use of the drug is not recommended and the patient has a higher risk of developing adverse effects or exacerbation of their existing health condition, however, use is still possible and the risk may be outweighed by other considerations.</mims:d2h_sev_level>
	</mims:d2h_sev_levels>
	<mims:d2l_sev_levels>
		<mims:d2l_sev_level name="1">Human experience may or may not be available but these drugs can have severe toxicity to a nursing infant. The drug should not be given to a nursing mother. These drugs also include those that are indicated in conditions where lactation is a contraindication.</mims:d2l_sev_level>
		<mims:d2l_sev_level name="2">These drugs have been reported to cause clinically significant side effects to a nursing infant, and/or existing animal data suggest significant risks to a nursing infant. These agents may also inhibit lactation or may harm the mother. If use during breastfeeding is unavoidable, monitoring is needed for possible toxicity to either mother or infant.</mims:d2l_sev_level>
	...
	<mims:doc_levels>
		<mims:doc_level name="4">There have been several published reports of this interaction. The pharmacological explanation of why the interaction occurs is well documented and understood.There are usually controlled studies that have established that the interaction exists.</mims:doc_level>
		<mims:doc_level name="3">Although controlled studies may not have been performed, several case reports have been documented and other data strongly suggests this interaction exists.</mims:doc_level>
		<mims:doc_level name="2">Few reports of this interaction exist. These few reports usually consist of limited case reports where clinically sound justification of the interaction is found.</mims:doc_level>
		<mims:doc_level name="1">The interaction may have occurred with other medicines within the same class, or there is a theoretical possibility that the interaction exists.</mims:doc_level>
	</mims:doc_levels>
	<mims:preg_cats>
		...
		<xsl:choose>
			<xsl:when test="count(*)=0">
				<div class="ui-widget">
					<div class="ui-state-default ui-corner-all warning-bar">
						<p>
							<span class="ui-icon ui-icon-alert"/>No prescribing information available.</p>
					</div>
				</div>
			</xsl:when>
			<xsl:otherwise>
  ...
  <span>
    ...
    <xsl:value-of select="//Result/@expiryDate"/>
						...
						<xsl:if test="//VIDAL">
							...
							<li>
								<a href="#tab-pils-en">PILS - EN</a>
							</li>
						</xsl:if>
						<xsl:if test="//PILS/SPECIFICPIL">
							<li>
								<a href="#tab-pils-bm">PILS - BM</a>
							</li>
						</xsl:if>
					...
					-->
					<xsl:if test="//PILS/SPECIFICPIL[@Language='English']">
    <div id="tab-pils-en">
        <xsl:apply-templates select="//PILS/SPECIFICPIL[@Language='English']"/>
    </div>
				...
				<!--<xsl:if test="//PILS/SPECIFICPIL">
					...
					<xsl:if test="//PILS/SPECIFICPIL[@Language='BahasaMelayu']">
				<div id="tab-pils-bm">
				<xsl:apply-templates select="//PILS/SPECIFICPIL[@Language='BahasaMelayu']"/>
				</div>
...
<xsl:template match="//Detail/Product">
	<!--<xsl:if test="position() = 1">--> <!-- Add this condition to ensure uniqueness -->
	<h4 class="subheading">
...
<xsl:template match="PackageType[not(preceding::PackageType=.)]">
	...
	<xsl:template match="CautionaryLabel" mode="cals-details">
		<div class="cal-details">
			<p>
				<strong>
					<xsl:value-of select="@type"/> Warning:</strong>
				<xsl:value-of select="Warning"/>
			</p>
  ...
  <!-- This only works with FastTrack Linux/Mac, where image data is returned as base64 
  <xsl:template match="ImageData">
    <xsl:element name="img">
      <xsl:attribute name="src">
        data:<xsl:value-of select="./@mimeType"/>;base64,<xsl:value-of select="text()"/>
      </xsl:attribute>
    </xsl:element>
  </xsl:template>-->
</xsl:stylesheet>